...
($ => {
  // const explore_image = $(".explore-section").find("img:visible")[0];
  // $(".explore-image-wrapper").css("background-image", `url(${explore_image.src})`);
  $(window).on("resize", function () {
    if (window.innerWidth >= 1200) {
      $(".mega-menu").css({
        height: window.innerHeight - $("header").outerHeight(true),
        "max-height": window.innerHeight - $("header").outerHeight(true)
      });
    }
  }).trigger("resize"); // Mobile - Hamburger menu

  const hamburger_menu = $(".hamburger-menu-wrapper");
  ...
  $(".main-menu-wrapper .btn").on("click", function (e) {
    e.preventDefault();
    $(this).closest("ul").find(".btn").not($(this)).removeClass("active");
    $(this).closest("ul").find(".sub-menu").not($(this).next(".sub-menu")).slideUp();
    $(this).toggleClass("active");
    $(this).next(".sub-menu").slideToggle();
  }); // collapse all sub-menu items

  if (window.innerWidth < 1200) {
    $(".main-menu-wrapper .sub-menu").css("display", "none");
  }

  $(".floating-menu .has-sub-menu").on("click", function (e) {
    $(this).next(".sub-menu").toggle();
  });
  $(document).on("click", function (e) {
    if ($(".floating-menu .has-sub-menu").closest("li") !== e.target && !$(".floating-menu .has-sub-menu").closest("li").has(e.target).length && window.innerWidth < 768) {
      $(".floating-menu .sub-menu").hide();
    }
  }); // Header search box

  $('.center-menu-wrapper form [type="submit"]').on("click", function (e) {
    if (!$(this).closest("form").hasClass("open")) {
      e.preventDefault();
      $(this).closest("form").find(".form-item .form-text").trigger("focus");
      $(this).closest("form").addClass("open");
    }

    if (window.innerWidth < 1200) {
      $(this).closest("form").find(".form-item").css("top", $("header").height());
      $(this).closest("form").find(".form-item").slideToggle("fast");
    }
  });
  $(document).on("click", function (e) {
    if ($(e.target).closest(".center-menu-wrapper form").length == 0) {
      $(".center-menu-wrapper form").removeClass("open");

      if (window.innerWidth < 1200) {
        $(".center-menu-wrapper form").find(".form-item").slideUp("fast");
      }
    }
  });
  /** Popups */
  ...
  swiper__WEBPACK_IMPORTED_MODULE_2__.default.use([swiper__WEBPACK_IMPORTED_MODULE_3__.default, swiper__WEBPACK_IMPORTED_MODULE_4__.default, swiper__WEBPACK_IMPORTED_MODULE_5__.default]); // Banner slider

  new swiper__WEBPACK_IMPORTED_MODULE_2__.default(".banner-slider .swiper-container", {
    loop: false,
    centeredSlides: false,
    slideToClickedSlide: false,
    observer: true,
  ...
  new swiper__WEBPACK_IMPORTED_MODULE_2__.default(".product-categories-slider .swiper-container", {
    ...
    pagination: {
      el: $(".product-categories-slider .swiper-pagination")[0],
      clickable: true
    },
    navigation: {
      nextEl: $(".product-categories-slider .swiper-button-next")[0]
    }
  }); // Banner slider

  const blockSlider = [];
  $.each($(".block-slider.swiper-container"), function (_, e) {
    blockSlider.push(new swiper__WEBPACK_IMPORTED_MODULE_2__.default(e, {
      slideToClickedSlide: true,
      slidesPerView: 1.1,
      loop: false,
      breakpoints: {
  ...
  }); // Product listing category slider

  new swiper__WEBPACK_IMPORTED_MODULE_2__.default(".category-box-wrapper .swiper-container", {
    slideToClickedSlide: true,
    loop: false,
    slidesPerView: 3,
    spaceBetween: 20
  ...
  const milestoneSwiper = new swiper__WEBPACK_IMPORTED_MODULE_2__.default(".years-container .swiper-container", {
    slideToClickedSlide: true,
    loop: false,
    slidesPerView: 1,
    centeredSlides: true,
    ...
    on: {
      init: function () {
        ...
        $(".years-container").find(".swiper-slide-active").addClass("active");
      }
    }
  }).on("slideChange", function (swiper) {
    $(".years-container").find(".swiper-slide").removeClass("active");
    $(swiper.slides[swiper.activeIndex]).addClass("active");
  });
  $(".years-container .swiper-button-next").on("click", function (e) {
    milestoneSwiper.slideNext();
    $("html, body").animate({
      scrollTop: $($(milestoneSwiper.slides[milestoneSwiper.activeIndex]).find("a").attr("href")).offset().top - (window.innerWidth < 1024 ? 100 : 0)
    }, 300);
  });
  $(".years-container .swiper-button-prev").on("click", function (e) {
    milestoneSwiper.slidePrev();
    console.log(milestoneSwiper);
    console.log(milestoneSwiper.slides[milestoneSwiper.activeIndex]);
    $("html, body").animate({
      scrollTop: $($(milestoneSwiper.slides[milestoneSwiper.activeIndex]).find("a").attr("href")).offset().top - (window.innerWidth < 1024 ? 100 : 0)
    }, 300);
  });
  $(".years-container").on("click", "a", function (e) {
    e.preventDefault();
    $("html, body").animate({
      scrollTop: $($(e.target).attr("href")).offset().top - (window.innerWidth < 1024 ? 100 : 0)
    }, 300);
    $(".years-container .swiper-slide").removeClass("active");
    $(this).closest(".swiper-slide").addClass("active");
  });
  $(document).on("touchmove DOMMouseScroll mousewheel wheel", function (e) {
    var scrollDistance = $(window).scrollTop(); // Assign active class to nav links while scolling

    $.each($(".milestone"), function (i, e) {
      const eTop = Math.ceil($(this).offset().top) - 120,
            eBottom = eTop + $(this).outerHeight(),
            wrapper_top = $(".milestones-container").offset().top + 64;

      if (eTop <= scrollDistance && eBottom >= scrollDistance) {
        console.log(scrollDistance, wrapper_top);

        if (window.innerWidth < 1024) {
          milestoneSwiper.slideTo(scrollDistance < wrapper_top ? 1 : i + 1, false, false);
          milestoneSwiper.update();
          $(".years-container").find(".swiper-slide").removeClass("active");
          $(milestoneSwiper.slides[scrollDistance < wrapper_top ? 1 : i + 1]).addClass("active");
        } else {
          setTimeout(() => {
            $(".years-container").find(".swiper-slide").removeClass("active");
            $(milestoneSwiper.slides[i]).addClass("active");
            milestoneSwiper.slideTo(i, false, false);
            milestoneSwiper.update();
          }, 320);
        }
  ...
  $(".tabs-container .nav-tabs a").on("click", function (e) {
    e.preventDefault();
    $(".tabs-container .nav-tabs a").removeClass("active");
    $(this).addClass("active");
    $(".tabs-container .tab-content .tab-pane").removeClass("active");
    $(".tabs-container").find($(this).attr("href")).addClass("active");
    $.each(blockSlider, function (_, e) {
      e.update();
    });
  });
  $(".tabs-container .tab-content .tab-title").on("click", function (e) {
    e.preventDefault();
    $(".tabs-container .tab-content .tab-title").removeClass("active");
    $(this).addClass("active");
    $(".tabs-container .tab-content .tab-pane").removeClass("active");
  ...
  $(".product-category-filter .title .btn").on("click", function (e) {
    $(this).closest("ul").find("li").not($(this).closest("li")).removeClass("open");
    $(this).closest("li").toggleClass("open");
  }); // OTP BOX

  $(".input-otp").on("keydown", stopCarret);
  $(".input-otp").on("keyup", stopCarret);

  function stopCarret(e) {
    if (e.target.value.length >= e.target.getAttribute("maxlength") || 4) {
  ...
  $(".accordion-head button").on("click", function (e) {
    const isFirst = $(this).closest(".accordion-container").find(".accordion.active").length === 0;
    $(this).closest(".accordion").toggleClass("active");
    $(this).closest(".accordion-container").find(".accordion").not($(this).closest(".accordion")).removeClass("active");
    $(this).closest(".accordion-container").find(".accordion").not($(this).closest(".accordion")).find(".accordion-body").slideUp();
    $(this).closest(".accordion").find(".accordion-body").slideToggle(300, "", function () {
      if (!isFirst) {
        $("html, body").animate({
          scrollTop: $(e.target.parentElement).offset().top - 100
        }, 400);
      }
    });
  });
  $(".accordion").first().find("button").trigger("click");
  /**
